...
def get_filters(mySQLcursor):
    ...
    #st.write(movies_df.head(5))
    movies_df.drop(['Certificate'], axis=1,inplace=True)

    # col1, col2 = st.columns([1,3] )
    # data_movies = db_data.get_table_data(mySQLcursor, 'movies')
    # movies_df = pd.DataFrame(data_movies, columns=column_names)
    # valid_rated_movies_df = movies_df[movies_df["Duration"] > 0]
    # #st.write(movies_df)
    # if (movies_df.shape[0] > 0):
    #     with col1:
    #         genre_list = list(movies_df["Genre"].unique())
    #         selected_genre = st.selectbox('Select a Genre', genre_list)
    #         #st.write(selected_genre)
    #         voting_list = list(movies_df["Voting"].unique())
    #         selected_voting = st.selectbox('Select a voting', voting_list)
    #         duration_filter = st.sidebar.radio("⏳ Select Duration:", ["All", "< 2 hrs", "2-3 hrs", "> 3 hrs"])
    #     with col2:
    #         mov_df = movies_df[movies_df['Voting']>0]
    #         st.write(mov_df['Voting'].min())
    #         st.write(movies_df['Voting'].max())
    #         st.write(movies_df['Voting'].mean())
    #         filtered_genre_list = movies_df[movies_df['Genre']==selected_genre]
    #         filtered_genre_list = filtered_genre_list['Title']
    #         st.write(filtered_genre_list.reset_index(drop=True))
    #         filtered_voting_list = movies_df[movies_df['Voting']==selected_voting]
    #         st.write(filtered_voting_list.title.to_list())
    # else:
